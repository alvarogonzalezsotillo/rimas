...
function alternaPalabraODatos(event){
    ...
    if( expanded == "true" ){
        ...
        const silabas = pronunciacionHTML(palabra);
        div.innerHTML = `${palabra} se pronuncia <br> ${silabas}`;
...
function  indicaPorcentaje(indice,total){
    ...
    if( !indice && !total ){
        ...
        porcentaje.innerHTML = `Buscando rimas para ${palabra} (${pronunciacionHTML(palabra)}) ${indice}/${total}`;
